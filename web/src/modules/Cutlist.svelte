<script>
  ...
  async function doExport() {
    ...
    // iOS Safari ignores initial-scale resets when user-scalable=no is active.
    // Briefly allow scaling so iOS processes the scale=1 reset, then re-lock.
    const vp = document.querySelector('meta[name="viewport"]');
    if (vp) {
      const orig = vp.getAttribute('content');
      vp.setAttribute('content', 'width=device-width, initial-scale=1, minimum-scale=1, maximum-scale=5, user-scalable=yes');
